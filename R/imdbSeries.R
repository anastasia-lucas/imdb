...
#' @export
imdbSeries<-function(seriesname, apikey, seasons = 1) {
        if(missing(apikey)){stop("Please specify OMDB apikey")}
        ...
        for( i in seasons) {
                link<-gsub(pattern = " ", replacement = "%20", x=(paste("http://www.omdbapi.com/?apikey=", apikey, "&t=", seriesname,"&Season=",i, sep = "")))
